...
钉钉直播回放下载工具：Selenium 自动化浏览器抓取登录态 Cookie，从浏览器性能日志中解析 m3u8 链接，再调用本地 `N_m3u8DL-RE.exe` 子进程下载视频。
...
```bash
# 安装依赖（只用 requirements.txt，没有 pyproject.toml）
pip install -r requirements.txt

# 启动程序（入口在 main.py 的 main()）
python -m src.dingtalk_downloader.main

# 运行测试（README 中给出的命令，但当前仓库没有 tests/ 目录，
# 若新增单元测试需放在 tests/unit/ 下，pytest 会自动发现）
pytest tests/unit/ -v
...
- `core/m3u8_download_service.py` — 组合 `M3u8Parser` + `M3u8FileManager`，把 m3u8 文件下载到 `temp/` 下用 UUID 命名的临时文件，完成后清理。
...
VideoDownloadManager.initialize_download
  ...
  └─ M3u8Parser + M3u8DownloadService → 从性能日志找 .m3u8，浏览器 JS fetch 拉到 temp/
VideoDownloadManager.process_video (重试最多 20 次)
  ├─ PathSelector.get_save_dir      → default/<tkinter 弹窗>
  └─ NM3u8DLRE.download             → subprocess 调 assets/bin/N_m3u8DL-RE.exe
```
...
- 临时文件由 `M3u8FileManager` 命名（UUID），`VideoDownloadManager.process_video` 的 `finally` 块会调用 `M3u8DownloadService.cleanup_temp_file` 删除；新流程如复用 m3u8 文件需自行管理生命周期。
- `assets/bin/` 下的 exe 在 Windows 下随仓库发布；非 Windows 平台需自行提供 `n_m3u8dl_re.executable_path` 与 `ffmpeg.executable_path`，命令构建逻辑不区分平台。
- 当前仓库没有 `tests/`、`pyproject.toml`、`pytest.ini`、`.cursor/`、`.cursorrules`、`.github/copilot-instructions.md`；如要补齐基础设施参考 README 中的 `pytest tests/unit/ -v` 命令。