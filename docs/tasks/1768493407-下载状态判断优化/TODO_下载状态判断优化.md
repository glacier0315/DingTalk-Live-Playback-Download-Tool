...
- [x] 修改subprocess.run()调用，捕获输出
- [x] 添加退出码检查逻辑
- [x] 添加输出解析逻辑，检查ERROR:和Failed关键字
- [x] 添加错误信息提取和日志输出逻辑
- [x] 添加成功日志输出逻辑
- [x] 编写8个新的测试用例
- [x] 修复5个旧的测试用例
- [x] 运行所有测试，验证通过
- [x] 修复ffmpeg_wrapper.py缺少logger导入的问题
...
**方法1：运行单元测试**
```powershell
pytest tests/unit/test_n_m3u8dl_re.py -v
```
...
**方法2：添加调试日志**
```python
# 在n_m3u8dl_re.py中添加调试日志
logger.debug(f"退出码: {result.returncode}")
logger.debug(f"标准输出: {result.stdout}")
logger.debug(f"标准错误: {result.stderr}")
logger.debug(f"合并输出: {output}")
```

**方法3：使用断点调试**
```python
# 在n_m3u8dl_re.py中添加断点
