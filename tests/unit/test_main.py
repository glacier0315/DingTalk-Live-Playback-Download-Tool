...
def test_single_mode_with_default_options():
    ...
    with patch('builtins.input') as mock_input, \
         patch('dingtalk_downloader.main.Downloader') as mock_downloader_class:
...
def test_batch_mode_with_default_options():
    """测试批量下载模式 - 使用默认选项"""
    with patch('builtins.input') as mock_input, \
         patch('dingtalk_downloader.main.FileReader') as mock_file_reader_class, \
         patch('dingtalk_downloader.main.Downloader') as mock_downloader_class:
...
def test_batch_mode_with_manual_options():
    ...
    with patch('builtins.input') as mock_input, \
        ...
        mock_file_reader = Mock()
        mock_file_reader.read_links.return_value = {
            "Sheet1": ["https://n.dingtalk.com/test1"]
        }
        mock_file_reader_class.return_value = mock_file_reader
...
def test_main_single_mode():
    """测试主程序入口 - 单个视频下载模式"""
    with patch('builtins.input') as mock_input, \
         patch('dingtalk_downloader.main.single_mode') as mock_single_mode:
...
def test_main_batch_mode():
    """测试主程序入口 - 批量下载模式"""
    with patch('builtins.input') as mock_input, \
         patch('dingtalk_downloader.main.batch_mode') as mock_batch_mode:
