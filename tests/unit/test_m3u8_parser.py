...
def mock_browser_with_logs():
    ...
    browser.extract_m3u8_links_from_logs.return_value = [
        "https://test.com/live_hp/123/test.m3u8?uuid=abc"
    ]
    return browser
...
def test_m3u8_parser_fetch_m3u8_links_chrome(mock_browser_with_logs):
    ...
    mock_browser_with_logs.extract_m3u8_links_from_logs.return_value = [
        "https://test.com/live_hp/456/test.m3u8?liveUuid=def"
    ]
...
def test_m3u8_parser_fetch_m3u8_links_log_exception_handling(mock_browser):
    """测试日志处理异常处理"""
    mock_browser.get_log.return_value = [{"message": "invalid json"}]
...
def test_m3u8_parser_fetch_m3u8_links_json_parse_error(mock_browser):
    """测试JSON解析错误"""
    mock_browser.get_log.return_value = [{"message": "invalid json without quotes"}]
...
def test_m3u8_parser_fetch_m3u8_links_empty_json_url(mock_browser):
    """测试JSON中URL为空的情况"""
    mock_browser.get_log.return_value = [{"message": '{"url":""}'}]
...
def test_m3u8_parser_fetch_m3u8_links_with_special_characters(mock_browser):
    ...
    mock_browser.extract_m3u8_links_from_logs.return_value = [
        "https://test.com/live_hp/123/test.m3u8?liveUuid=abc&token=xyz-123"
    ]
